...
for (let i = 0, len = GitProjects.length; i < len; i++) {
	...
	try {
		await within(async () => {
			...
			const author =
				argv.author ||
				(argv.committer ? "" : await $`git config user.name`).stdout.trim();
			...
			const flags = [
				...
				`--author=${author}`,
				`--committer=${committer}`,
				`--pretty=format:"{_'_text_'_:_'_%s_'_,_'_hash_'_:_'_%h_'_,_'_author_'_:_'_%an_'_,_'_timestamp_'_:%ct}"`,
				`--all`,
			];
...
Object.values(logObj).forEach(({ text, logs }) => {
	...
	Object.keys(logs).forEach((key) => {
		...
		for (const log of children) {
			if (style === 0) {
				// 日期：描述
				result += `${dayjs(log.timestamp * 1000).format("YYYY-MM-DD")}：${
					log.text
				}\n`;
			} else if (style === 3) {
				// 时间：项目：描述
				result += `${dayjs(log.timestamp * 1000).format("HH:mm")} (${
					log.project
				}) ：${log.text}\n`;
			} else {
				result += `${log.text}\n`;
